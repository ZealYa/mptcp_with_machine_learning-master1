...
void InstallOnOffApplications(NodeContainer& servers, NodeContainer& clients, uint32_t packetSize,uint32_t mptcpApp)
{
  // Create and install the applications on the server and client
  static int mptcpAppNum = mptcpApp;
  std::cout << "mptcp apps num: " << mptcpApp << endl;
  ...
  for(int i = 0; i < servers.GetN();i++){
    ...
    if(mptcpAppNum > 0){
      ...
      MpOnOff->SetAttribute("DataRate", DataRateValue (DataRate("5Mbps")));
...
void InstallMultiOnOffApplications(NodeContainer& servers, NodeContainer& clients, uint32_t packetSize,uint32_t mptcpApp)
{
  // Create and install the applications on the server and client
  static int mptcpAppNum = mptcpApp;
  std::cout << "mptcp apps num: " << mptcpApp << endl;
